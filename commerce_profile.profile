...
function commerce_profile_install_tasks($install_state) {
  ...
  $tasks['commerce_profile_enable_nodes_with_pictures_module'] = array(
    'display' => FALSE,
  );
  return $tasks;
}
 ...
 * 
 */
function commerce_profile_enable_nodes_with_pictures_module() {
  if (!module_exists('nodes_with_pictures')) {
    $modules[] = ('nodes_with_pictures');
  }
  module_enable($modules);
}
